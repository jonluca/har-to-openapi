...
describe("Spec generator", async () => {
  ...
  test(`Returns one spec for a har with two domains and forceAllRequestsInSameSpec set to true, and adds servers to path`, async ({
    expect,
  }) => {
    const data = await generateSpecs(har, { forceAllRequestsInSameSpec: true, addServersToPaths: true });
    expect(data).toHaveLength(1);
    expect(data).toMatchSnapshot();
  });
  test(`Returns two specs for a har with two domains and forceAllRequestsInSameSpec set to false, and adds servers to path`, async ({
    expect,
  }) => {
    const data = await generateSpecs(har, { forceAllRequestsInSameSpec: false, addServersToPaths: true });
    expect(data).toHaveLength(2);
    expect(data).toMatchSnapshot();
  });
});
