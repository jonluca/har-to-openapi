...
import { basePath, sampleHar, typedParamsHar } from "./test-utils.js";
...
`,
  ...
  test("supports OpenAPI version selection and parameter inference flags", async ({ expect }) => {
    const harness = createHarness({
      [path.join("/virtual", "typed.har")]: JSON.stringify(typedParamsHar()),
    });
    ...
    const exitCode = await runCli(
      [
        ...
        "--min-length-for-numeric-path",
        "1",
        "--no-infer-parameter-types",
      ],
      harness.dependencies,
    );
    const output = JSON.parse(harness.readOutput());
    ...
    expect(output.paths["/orders/{id}"].parameters[0].schema.type).toBe("string");
    expect(output.paths["/orders/{id}"].get.parameters[0].schema.type).toBe("string");
  });
});
